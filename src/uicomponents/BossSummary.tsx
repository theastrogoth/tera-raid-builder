...
import { getItemSpriteURL, getPokemonArtURL, getTypeIconURL, getTeraTypeBannerURL, arraysEqual, getTranslation } from "../utils";
...
function BossSummary({pokemon, setPokemon, results, displayedTurn, allSpecies, allMoves, setAllSpecies, setAllMoves, prettyMode, translationKey}: 
    {pokemon: Raider, setPokemon: (r: Raider) => void, results: RaidBattleResults, displayedTurn: number, allSpecies: Map<SpeciesName,PokemonData> | null, allMoves: Map<MoveName,MoveData> | null, prettyMode: boolean, translationKey: any
        ...
        <Box>
            <Paper elevation={3} sx={{ mx: 1, my: 1, width: 530, display: "flex", flexDirection: "column", padding: "0px"}}>                
                <Stack direction="column" spacing={0} alignItems="center" justifyContent="top" minHeight={prettyMode ? undefined : "600px"} sx={{ marginTop: 1 }} >
                    ...
                    <Stack direction="row" spacing={0} alignItems="center" justifyContent="center" sx={{ width: "100%", paddingTop: "5px" }}>
                        <Box width="100%" marginTop="10px" display="flex" justifyContent="center" sx={{ paddingLeft: "60px", paddingBottom: "50px"}}>
                            ...
                            <Box position="relative">
                                ...
                                <Box position="absolute" sx={{ bottom: "-50px", right: "-5px", filter: "drop-shadow(0px 0px 2px rgba(0, 0, 0, .5))"}}>
                                    <img
                                        height="45px"
                                        src={getTeraTypeBannerURL(pokemon.teraType || "blank")}
                                        onError={({ currentTarget }) => {
                                            currentTarget.onerror = null; // prevents looping
                                            currentTarget.src=getTeraTypeBannerURL("blank");
                                        }}
                                        alt=""
                                    />
                                </Box>
                    ...
                    <Stack direction="row" spacing={-2} >
                        ...
                        <Box flexGrow={1} />
                        <Stack direction="column" spacing={0} justifyContent="start" alignItems="start" sx={{ width: "300px", minHeight:( prettyMode ? undefined : "375px") }}>
                            <BossBuildControlsMemo moveSet={moveSet} pokemon={pokemon} setPokemon={setPokemon} results={results} displayedTurn={displayedTurn} allMoves={allMoves} prettyMode={prettyMode} translationKey={translationKey}/>
                        </Stack>
                    </Stack>
                </Stack>
            </Paper>
...
export default React.memo(BossSummary,
    (prevProps, nextProps) => (
        ...
        prevProps.displayedTurn === nextProps.displayedTurn &&
